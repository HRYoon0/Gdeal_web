// Swiper.js Creative 효과 적용 스크립트 v14
// 2x2 레이아웃 + 배경 블러 기법 + React 원본 애니메이션 완전 제거
(function() {
  'use strict';

  console.log('🎨 Swiper Creative 스크립트 v14 시작 (배경 블러 기법)');

  // Paperlogy 폰트 @font-face 정의 (눈누 CDN)
  const fontStyle = document.createElement('style');
  fontStyle.id = 'paperlogy-font';
  fontStyle.textContent = `
    /* Paperlogy 폰트 @font-face 정의 */
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-1Thin.woff2') format('woff2'); font-weight: 100; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-2ExtraLight.woff2') format('woff2'); font-weight: 200; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-3Light.woff2') format('woff2'); font-weight: 300; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-4Regular.woff2') format('woff2'); font-weight: 400; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-5Medium.woff2') format('woff2'); font-weight: 500; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-6SemiBold.woff2') format('woff2'); font-weight: 600; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-7Bold.woff2') format('woff2'); font-weight: 700; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-8ExtraBold.woff2') format('woff2'); font-weight: 800; font-style: normal; font-display: swap; }
    @font-face { font-family: 'Paperlogy'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/2408-3@1.0/Paperlogy-9Black.woff2') format('woff2'); font-weight: 900; font-style: normal; font-display: swap; }

    /* Paperlogy 폰트 전역 적용 */
    *, *::before, *::after {
      font-family: 'Paperlogy', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif !important;
    }

    body {
      font-family: 'Paperlogy', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif !important;
      -webkit-font-smoothing: antialiased;
      -moz-osx-font-smoothing: grayscale;
    }
  `;
  document.head.appendChild(fontStyle);

  // 즉시 React 슬라이드쇼 숨기기 (zoom 효과 방지)
  const immediateStyle = document.createElement('style');
  immediateStyle.id = 'swiper-immediate-hide';
  immediateStyle.textContent = `
    /* React 슬라이드쇼 즉시 숨김 - Swiper 초기화 전 zoom 효과 방지 */
    .relative.bg-gray-200.rounded-2xl > .relative {
      opacity: 0 !important;
      visibility: hidden !important;
    }
  `;
  document.head.appendChild(immediateStyle);

  // 2x2 레이아웃 스타일 즉시 적용
  const layoutStyle = document.createElement('style');
  layoutStyle.id = 'layout-2x2';
  layoutStyle.textContent = `
    /* 메인 그리드를 1열로 변경 - 각 섹션이 전체 너비 차지 */
    .max-w-full > .grid.lg\\:grid-cols-2 {
      grid-template-columns: 1fr !important;
    }

    /* 교육활동(두 번째)을 위로, 최신소식(첫 번째)을 아래로 */
    .max-w-full > .grid > .col-span-1:first-child {
      order: 2 !important;
    }
    .max-w-full > .grid > .col-span-1:last-child {
      order: 1 !important;
    }

    /* 섹션 간격 늘리기 */
    .max-w-full > .grid.lg\\:grid-cols-2 {
      gap: 3.5rem !important;
    }

    /* 타이틀 헤더 컨테이너 */
    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 {
      padding: 1rem 1.5rem !important;
      background: linear-gradient(135deg, rgba(255,255,255,0.9) 0%, rgba(248,250,252,0.95) 100%) !important;
      backdrop-filter: blur(10px) !important;
      border-radius: 1rem !important;
      box-shadow: 0 4px 20px rgba(0,0,0,0.04), 0 1px 3px rgba(0,0,0,0.02) !important;
      border: 1px solid rgba(255,255,255,0.8) !important;
      margin-bottom: 1.5rem !important;
    }

    /* 타이틀 텍스트 */
    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 h2 {
      position: relative !important;
      font-size: 1.5rem !important;
      font-weight: 700 !important;
      letter-spacing: -0.02em !important;
      color: #1a1a2e !important;
      padding-left: 1rem !important;
    }

    /* 타이틀 앞 세로 그라데이션 바 */
    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 h2::before {
      content: '' !important;
      position: absolute !important;
      left: 0 !important;
      top: 50% !important;
      transform: translateY(-50%) !important;
      width: 4px !important;
      height: 1.5rem !important;
      background: linear-gradient(180deg, #66ae7d 0%, #4ecdc4 100%) !important;
      border-radius: 2px !important;
    }

    /* 더보기 버튼 */
    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 a {
      display: inline-flex !important;
      align-items: center !important;
      gap: 0.4rem !important;
      padding: 0.6rem 1.2rem !important;
      background: linear-gradient(135deg, #66ae7d 0%, #4ecdc4 100%) !important;
      border-radius: 2rem !important;
      font-weight: 600 !important;
      font-size: 0.8rem !important;
      color: white !important;
      transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
      box-shadow: 0 4px 15px rgba(102, 174, 125, 0.3) !important;
      text-decoration: none !important;
    }

    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 a:hover {
      transform: translateY(-2px) scale(1.02) !important;
      box-shadow: 0 8px 25px rgba(102, 174, 125, 0.4) !important;
      background: linear-gradient(135deg, #4ecdc4 0%, #66ae7d 100%) !important;
    }

    /* 각 섹션 내부를 2열 그리드로 변경 */
    .max-w-full > .grid > .col-span-1 > .relative {
      display: grid !important;
      grid-template-columns: 1fr 1fr !important;
      grid-template-rows: auto 1fr !important;
      gap: 1.5rem !important;
    }

    /* 헤더는 전체 너비 차지 */
    .max-w-full > .grid > .col-span-1 > .relative > .flex.items-center.justify-between.mb-6 {
      grid-column: 1 / -1 !important;
    }

    /* 슬라이드쇼 높이 조정 */
    .max-w-full > .grid > .col-span-1 > .relative > .relative.bg-gray-200.rounded-2xl {
      height: 38rem !important;
    }

    /* 리스트 컨테이너 스타일 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 {
      margin-top: 0 !important;
      height: 38rem !important;
      overflow-y: auto !important;
      background: linear-gradient(145deg, #ffffff 0%, #f8fafc 100%) !important;
      border-radius: 1.25rem !important;
      padding: 1.5rem !important;
      box-shadow: inset 0 2px 10px rgba(0,0,0,0.03) !important;
      border: 1px solid rgba(0,0,0,0.04) !important;
    }

    /* 커스텀 스크롤바 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3::-webkit-scrollbar {
      width: 6px !important;
    }
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3::-webkit-scrollbar-track {
      background: transparent !important;
    }
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3::-webkit-scrollbar-thumb {
      background: linear-gradient(180deg, #66ae7d 0%, #4ecdc4 100%) !important;
      border-radius: 3px !important;
    }

    /* 리스트 아이템 간격 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a {
      display: block !important;
      margin-bottom: 1.25rem !important;
    }
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a:last-child {
      margin-bottom: 0 !important;
    }

    /* 리스트 아이템 카드 스타일 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a > div.flex {
      background: white !important;
      border-radius: 1rem !important;
      padding: 1rem 1.25rem !important;
      box-shadow: 0 2px 8px rgba(0,0,0,0.04), 0 1px 2px rgba(0,0,0,0.02) !important;
      border: 1px solid rgba(0,0,0,0.03) !important;
      transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
    }

    /* 리스트 아이템 호버 효과 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a > div.flex:hover {
      transform: translateY(-2px) !important;
      box-shadow: 0 8px 25px rgba(102, 174, 125, 0.15), 0 4px 10px rgba(0,0,0,0.05) !important;
      border-color: rgba(102, 174, 125, 0.2) !important;
    }

    /* 활성 리스트 아이템 스타일 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a > div.flex.bg-white.border-l-4 {
      background: linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%) !important;
      border-left: 4px solid #66ae7d !important;
      box-shadow: 0 8px 25px rgba(102, 174, 125, 0.2), 0 4px 10px rgba(0,0,0,0.05) !important;
      transform: translateY(-2px) scale(1.01) !important;
    }

    /* React 기본 화살표 아이콘 숨기기 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a > div.flex > div.text-gdeal-primary {
      display: none !important;
    }

    /* 리스트 아이템 position relative 추가 */
    .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 > a > div.flex {
      position: relative !important;
    }

    /* 최신소식 (첫 번째) - 화살표 오른쪽 */
    .max-w-full > .grid > .col-span-1:first-child > .relative > .mt-6.space-y-3 > a > div.flex.bg-white.border-l-4::after {
      content: '' !important;
      position: absolute !important;
      right: 1rem !important;
      top: 50% !important;
      width: 8px !important;
      height: 8px !important;
      border-top: 2px solid #66ae7d !important;
      border-right: 2px solid #66ae7d !important;
      transform: translateY(-50%) rotate(45deg) !important;
    }

    /* 교육활동 (두 번째) - 화살표 왼쪽 */
    .max-w-full > .grid > .col-span-1:last-child > .relative > .mt-6.space-y-3 > a > div.flex.bg-white.border-l-4::after {
      content: '' !important;
      position: absolute !important;
      left: 1rem !important;
      top: 50% !important;
      width: 8px !important;
      height: 8px !important;
      border-top: 2px solid #66ae7d !important;
      border-left: 2px solid #66ae7d !important;
      transform: translateY(-50%) rotate(-45deg) !important;
    }

    /* 최신소식 섹션 (첫 번째) - 리스트 왼쪽, 슬라이드쇼 오른쪽 */
    .max-w-full > .grid > .col-span-1:first-child > .relative > .mt-6.space-y-3 {
      order: 1 !important;
    }
    .max-w-full > .grid > .col-span-1:first-child > .relative > .relative.bg-gray-200.rounded-2xl {
      order: 2 !important;
    }

    /* 교육활동 섹션 (두 번째) - 슬라이드쇼 왼쪽, 리스트 오른쪽 */
    .max-w-full > .grid > .col-span-1:last-child > .relative > .relative.bg-gray-200.rounded-2xl {
      order: 1 !important;
    }
    .max-w-full > .grid > .col-span-1:last-child > .relative > .mt-6.space-y-3 {
      order: 2 !important;
    }

    /* 모바일에서는 1열 레이아웃 유지 */
    @media (max-width: 1024px) {
      .max-w-full > .grid > .col-span-1 > .relative {
        grid-template-columns: 1fr !important;
      }
      .max-w-full > .grid > .col-span-1 > .relative > .relative.bg-gray-200.rounded-2xl,
      .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 {
        order: unset !important;
        height: auto !important;
      }
      .max-w-full > .grid > .col-span-1 > .relative > .relative.bg-gray-200.rounded-2xl {
        height: 24rem !important;
      }
      .max-w-full > .grid > .col-span-1 > .relative > .mt-6.space-y-3 {
        margin-top: 1.5rem !important;
        height: auto !important;
        max-height: 20rem !important;
      }
    }
  `;
  document.head.appendChild(layoutStyle);

  // Firebase 설정
  const FIREBASE_PROJECT_ID = 'gdeal-page-a67e2';
  const FIREBASE_API_KEY = 'AIzaSyBJsqUJK1AjhrLNzIY_79dIR2Mlg7zD09w';

  // Firebase Firestore REST API로 데이터 가져오기
  async function fetchFirestoreCollection(collection, orderBy = 'date', orderDir = 'DESCENDING', limit = 5, dateFilter = null) {
    const baseUrl = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents`;

    try {
      // 구조화된 쿼리 사용
      const queryUrl = `${baseUrl}:runQuery?key=${FIREBASE_API_KEY}`;

      const structuredQuery = {
        from: [{ collectionId: collection }],
        orderBy: [{ field: { fieldPath: orderBy }, direction: orderDir }],
        limit: limit
      };

      // 날짜 필터 추가 (trainings용: 오늘 이후 날짜만)
      if (dateFilter) {
        structuredQuery.where = {
          fieldFilter: {
            field: { fieldPath: 'date' },
            op: 'GREATER_THAN_OR_EQUAL',
            value: { stringValue: dateFilter }
          }
        };
      }

      const query = { structuredQuery };

      const response = await fetch(queryUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(query)
      });

      if (!response.ok) {
        throw new Error(`Firestore API 오류: ${response.status}`);
      }

      const data = await response.json();

      // Firestore 응답을 일반 객체로 변환
      return data
        .filter(item => item.document)
        .map(item => {
          const doc = item.document;
          const fields = doc.fields;
          const id = doc.name.split('/').pop();

          return {
            id,
            title: fields.title?.stringValue || '',
            description: fields.description?.stringValue || '',
            date: fields.date?.stringValue || '',
            location: fields.location?.stringValue || '',
            category: fields.category?.stringValue || '',
            images: fields.images?.arrayValue?.values?.map(v => v.stringValue) || []
          };
        });
    } catch (error) {
      console.error(`Firestore 데이터 로드 실패 (${collection}):`, error);
      return [];
    }
  }

  // Google Drive URL을 썸네일 URL로 변환
  function convertToThumbnailUrl(url) {
    if (!url) return '';
    const match = url.match(/https:\/\/drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)/);
    if (match && match[1]) {
      return `https://drive.google.com/thumbnail?id=${match[1]}&sz=w1000`;
    }
    return url;
  }

  // Swiper CSS 동적 로드
  const swiperCSS = document.createElement('link');
  swiperCSS.rel = 'stylesheet';
  swiperCSS.href = 'https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css';
  document.head.appendChild(swiperCSS);

  // 커스텀 스타일 추가
  const customStyle = document.createElement('style');
  customStyle.textContent = `
    /* Swiper가 적용된 컨테이너는 React 원본 요소 완전 숨김 */
    .relative.bg-gray-200.rounded-2xl:has(.swiper-creative-container) > :not(.swiper-creative-container) {
      display: none !important;
      opacity: 0 !important;
      visibility: hidden !important;
    }

    /* Swiper 컨테이너 스타일 */
    .swiper-creative-container {
      width: 100%;
      height: 100%;
      border-radius: 1rem;
      overflow: hidden;
      position: relative;
    }

    .swiper-creative-container .swiper {
      width: 100%;
      height: 100%;
    }

    .swiper-creative-container .swiper-slide {
      display: flex;
      align-items: center;
      justify-content: center;
      border-radius: 1rem;
      overflow: hidden;
      position: relative;
    }

    /* 슬라이드 내부 컨테이너 */
    .swiper-creative-container .swiper-slide .slide-inner {
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
      justify-content: center;
      overflow: hidden;
    }

    /* 배경 블러 이미지 */
    .swiper-creative-container .swiper-slide .slide-bg {
      position: absolute;
      inset: -20px;
      width: calc(100% + 40px);
      height: calc(100% + 40px);
      object-fit: cover;
      filter: blur(25px) brightness(0.7);
      transform: scale(1.1);
      z-index: 1;
    }

    /* 메인 이미지 (전체 보이기) */
    .swiper-creative-container .swiper-slide .slide-main {
      position: relative;
      max-width: 100%;
      max-height: 100%;
      width: auto;
      height: auto;
      object-fit: contain;
      z-index: 2;
      border-radius: 0.5rem;
      box-shadow: 0 10px 40px rgba(0,0,0,0.3);
    }

    /* Swiper 네비게이션 버튼 커스텀 */
    .swiper-creative-container .swiper-button-next,
    .swiper-creative-container .swiper-button-prev {
      width: 52px !important;
      height: 52px !important;
      background: rgba(255, 255, 255, 0.95) !important;
      border-radius: 50% !important;
      box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15), 0 2px 8px rgba(0, 0, 0, 0.1);
      transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
      opacity: 0;
    }

    .swiper-creative-container:hover .swiper-button-next,
    .swiper-creative-container:hover .swiper-button-prev {
      opacity: 1;
    }

    .swiper-creative-container .swiper-button-next:hover,
    .swiper-creative-container .swiper-button-prev:hover {
      transform: scale(1.1);
      box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2);
    }

    .swiper-creative-container .swiper-button-next::after,
    .swiper-creative-container .swiper-button-prev::after {
      font-size: 1.125rem !important;
      color: #66ae7d !important;
      font-weight: 700 !important;
    }

    /* Swiper 페이지네이션 */
    .swiper-creative-container .swiper-pagination {
      bottom: 1.25rem !important;
    }

    .swiper-creative-container .swiper-pagination-bullet {
      width: 10px !important;
      height: 10px !important;
      background: rgba(255, 255, 255, 0.5) !important;
      opacity: 1 !important;
      transition: all 0.3s ease !important;
      margin: 0 6px !important;
    }

    .swiper-creative-container .swiper-pagination-bullet-active {
      background: white !important;
      transform: scale(1.3);
      box-shadow: 0 2px 8px rgba(0,0,0,0.3);
    }

    /* 반응형 */
    @media (max-width: 768px) {
      .swiper-creative-container .swiper-button-next,
      .swiper-creative-container .swiper-button-prev {
        width: 40px !important;
        height: 40px !important;
        opacity: 1;
      }
    }

    /* 그라데이션 배경 (이미지 없을 때) */
    .slide-gradient-bg {
      background: linear-gradient(135deg, #66ae7d 0%, #497e56 50%, #3a6b47 100%);
    }
  `;
  document.head.appendChild(customStyle);

  // Swiper JS 동적 로드
  const swiperScript = document.createElement('script');
  swiperScript.src = 'https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js';
  document.body.appendChild(swiperScript);

  // 전역 Swiper 인스턴스 저장
  const swiperInstances = {};

  // 동기화 중 플래그 (무한 루프 방지)
  let isSyncing = false;

  // Swiper 로드 완료 후 초기화
  swiperScript.onload = function() {
    console.log('✅ Swiper.js 라이브러리 로드 완료');
    waitForSlideshowAndInit();
  };

  // 슬라이드쇼 렌더링 완료 대기 후 Swiper 적용
  function waitForSlideshowAndInit() {
    let checkCount = 0;
    const maxChecks = 100;

    const checkInterval = setInterval(() => {
      checkCount++;

      const newsSection = findSectionByTitle('최신 소식');
      const eventsSection = findSectionByTitle('교육 활동');

      let newsReady = isSectionReady(newsSection);
      let eventsReady = isSectionReady(eventsSection);

      if (newsReady || eventsReady) {
        clearInterval(checkInterval);
        console.log(`🎠 슬라이드쇼 준비 완료 (${checkCount * 100}ms 대기)`);

        setTimeout(() => {
          if (newsReady) initSwiperForSection(newsSection, 'news');
          if (eventsReady) initSwiperForSection(eventsSection, 'events');
        }, 500);
      }

      if (checkCount >= maxChecks) {
        clearInterval(checkInterval);
        console.log('⚠️ 슬라이드쇼 로딩 타임아웃');
      }
    }, 100);
  }

  // 섹션 준비 상태 확인
  function isSectionReady(section) {
    if (!section) return false;
    const container = section.querySelector('.relative.bg-gray-200.rounded-2xl');
    if (!container) return false;
    const hasSpinner = container.querySelector('.animate-spin');
    const hasImage = container.querySelector('img');
    const hasList = section.querySelector('.mt-6.space-y-3');
    return !hasSpinner && hasImage && hasList;
  }

  // 제목으로 섹션 찾기
  function findSectionByTitle(title) {
    const headers = document.querySelectorAll('h2');
    for (const header of headers) {
      if (header.textContent.trim() === title) {
        return header.closest('.col-span-1') || header.closest('.relative');
      }
    }
    return null;
  }

  // 섹션에 Swiper 적용 (Firebase 데이터 사용)
  async function initSwiperForSection(section, type) {
    const container = section.querySelector('.relative.bg-gray-200.rounded-2xl');
    if (!container || container.querySelector('.swiper-creative-container')) return;

    console.log(`📦 ${type} 섹션 Firebase 데이터 로드 시작`);

    // 기존 하단 리스트 찾기
    const listContainer = section.querySelector('.mt-6.space-y-3');
    if (!listContainer) {
      console.log(`⚠️ ${type}: 하단 리스트 없음`);
      return;
    }

    // Firebase에서 데이터 가져오기
    let firestoreData = [];
    if (type === 'events') {
      // 교육 활동: events 컬렉션에서 최신 5개 (날짜 내림차순)
      firestoreData = await fetchFirestoreCollection('events', 'date', 'DESCENDING', 5);
    } else if (type === 'news') {
      // 최신 소식: trainings (월별연수) 컬렉션에서 오늘 이후 날짜로 5개 (날짜 오름차순)
      const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD 형식
      firestoreData = await fetchFirestoreCollection('trainings', 'date', 'ASCENDING', 5, today);
    }

    if (firestoreData.length === 0) {
      console.log(`⚠️ ${type}: Firebase 데이터 없음, DOM에서 추출 시도`);
      // Fallback: DOM에서 데이터 추출
      const listItems = listContainer.querySelectorAll('a');
      listItems.forEach((item, index) => {
        const titleEl = item.querySelector('.text-sm.font-medium.text-gray-900');
        const title = titleEl?.textContent?.trim() || '';
        if (title) {
          firestoreData.push({
            id: index.toString(),
            title,
            date: '',
            location: '',
            category: '',
            images: []
          });
        }
      });
    }

    console.log(`📊 ${type}: Firebase에서 ${firestoreData.length}개 데이터 로드됨`);

    // 슬라이드 데이터 생성
    const slideData = firestoreData.map((item, index) => {
      // 첫 번째 이미지를 대표 이미지로 사용
      const imageUrl = item.images && item.images.length > 0
        ? convertToThumbnailUrl(item.images[0])
        : '';

      return {
        index,
        id: item.id,
        title: item.title,
        category: item.category || '',
        date: item.date ? item.date.replace(/-/g, '.') : '',
        location: item.location || '',
        description: item.description || '',
        image: imageUrl
      };
    });

    console.log(`🖼️ ${type}: 슬라이드 데이터 생성 완료`, slideData.map(s => ({ title: s.title, image: s.image ? '있음' : '없음' })));

    // Swiper 생성
    createSwiperFromData(container, listContainer, slideData, type);
  }

  // 수집된 데이터로 Swiper 생성
  function createSwiperFromData(container, listContainer, slideData, type) {
    if (container.querySelector('.swiper-creative-container')) return;

    // 기존 슬라이드쇼 영역만 Swiper로 교체
    const swiperHTML = createSwiperHTML(type, slideData);
    container.innerHTML = swiperHTML;

    // Swiper 초기화
    const swiperElement = container.querySelector('.swiper');
    if (swiperElement && window.Swiper) {
      const config = type === 'news' ? getNewsConfig() : getEventsConfig();

      // 슬라이드 변경 시 리스트 동기화
      config.on = {
        slideChange: function() {
          const realIndex = this.realIndex;
          syncListWithSwiper(listContainer, realIndex);
        },
        init: function() {
          // 초기화 시 첫 번째 아이템 활성화
          syncListWithSwiper(listContainer, 0);
        }
      };

      const swiper = new window.Swiper(swiperElement, config);
      swiperInstances[type] = swiper;

      // 리스트 아이템 클릭 시 Swiper 이동
      setupListClickEvents(listContainer, swiper);

      // React의 자체 상태 변경을 무시하기 위한 MutationObserver 설정
      setupReactOverride(listContainer, swiper);

      // Swiper 컨테이너 표시 (즉시 숨김 스타일 제거)
      container.style.opacity = '1';
      container.style.visibility = 'visible';

      console.log(`✨ ${type} Swiper Creative 효과 적용 완료`);
    }
  }

  // React 상태 변경 감지 및 Swiper 상태로 덮어쓰기
  function setupReactOverride(listContainer, swiper) {
    if (!listContainer) return;

    // MutationObserver로 React의 클래스 변경 감지
    const observer = new MutationObserver((mutations) => {
      // 동기화 중이면 무시 (무한 루프 방지)
      if (isSyncing) return;

      // 현재 Swiper 인덱스로 강제 동기화
      const currentIndex = swiper.realIndex;

      // 짧은 딜레이 후 동기화 (React 렌더링 완료 후)
      requestAnimationFrame(() => {
        syncListWithSwiper(listContainer, currentIndex);
      });
    });

    // 리스트 내 모든 div 요소의 class 속성 변경 감시
    const listItems = listContainer.querySelectorAll('a');
    listItems.forEach(item => {
      const innerDiv = item.querySelector('div.flex');
      if (innerDiv) {
        observer.observe(innerDiv, {
          attributes: true,
          attributeFilter: ['class']
        });
      }
    });

    console.log('🔒 React 상태 오버라이드 설정 완료');
  }

  // Swiper HTML 생성 (배경 블러 기법 적용)
  function createSwiperHTML(type, slides) {
    const slidesHTML = slides.map((slide, index) => {
      const hasImage = slide.image && !slide.image.includes('placeholder');

      return `
        <div class="swiper-slide ${!hasImage ? 'slide-gradient-bg' : ''}" data-index="${index}">
          ${hasImage ? `
            <div class="slide-inner">
              <img class="slide-bg" src="${slide.image}" alt="" aria-hidden="true">
              <img class="slide-main" src="${slide.image}" alt="${slide.title}" loading="lazy">
            </div>
          ` : ''}
        </div>
      `;
    }).join('');

    return `
      <div class="swiper-creative-container">
        <div class="swiper" id="${type}-swiper">
          <div class="swiper-wrapper">
            ${slidesHTML}
          </div>
          <div class="swiper-button-prev"></div>
          <div class="swiper-button-next"></div>
          <div class="swiper-pagination"></div>
        </div>
      </div>
    `;
  }

  // 리스트와 Swiper 동기화 - 기존 React 클래스 정확히 복원
  function syncListWithSwiper(listContainer, activeIndex) {
    if (!listContainer) return;

    // 동기화 시작 플래그 설정
    isSyncing = true;

    const listItems = listContainer.querySelectorAll('a');

    listItems.forEach((item, index) => {
      // 내부 div 요소 찾기 (실제 스타일이 적용된 요소)
      const innerDiv = item.querySelector('div.flex');
      if (!innerDiv) return;

      // 기본 클래스 (항상 유지)
      const baseClasses = 'flex items-center space-x-4 p-3 rounded-xl cursor-pointer transition-all duration-300';

      // 활성/비활성 클래스
      const activeClasses = 'bg-white border-l-4 border-gdeal-primary shadow-lg';
      const inactiveClasses = 'hover:bg-gray-50 hover:shadow-sm';

      if (index === activeIndex) {
        // Active 상태: 활성 클래스 적용
        innerDiv.className = `${baseClasses} ${activeClasses}`;
      } else {
        // 비활성 상태: hover 클래스 적용
        innerDiv.className = `${baseClasses} ${inactiveClasses}`;
      }
    });

    // 동기화 완료 후 플래그 해제 (다음 프레임에서)
    requestAnimationFrame(() => {
      isSyncing = false;
    });
  }

  // 리스트 클릭 이벤트 설정
  function setupListClickEvents(listContainer, swiper) {
    if (!listContainer) return;

    const listItems = listContainer.querySelectorAll('a');

    listItems.forEach((item, index) => {
      // 클릭 이벤트 추가 (기존 이벤트 유지하면서)
      item.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();

        // Swiper 슬라이드 이동
        swiper.slideToLoop(index, 700);

        console.log(`🖱️ 리스트 클릭: ${index}번째 슬라이드로 이동`);
      }, true); // capture 모드로 먼저 처리
    });
  }

  // 최신 소식: 회전 + 이동 (좌우 125% 이동 + Z축 20도 회전)
  function getNewsConfig() {
    return {
      effect: 'creative',
      grabCursor: true,
      loop: true,
      speed: 700,
      autoplay: {
        delay: 4000,
        disableOnInteraction: false,
        pauseOnMouseEnter: true,
      },
      creativeEffect: {
        perspective: true,
        limitProgress: 2,
        prev: {
          translate: ['-125%', 0, 0],
          rotate: [0, 0, -20],  // Z축 -20도 회전
          opacity: 0,
        },
        next: {
          translate: ['125%', 0, 0],
          rotate: [0, 0, 20],  // Z축 20도 회전
          opacity: 0,
        },
      },
      pagination: {
        el: '.swiper-pagination',
        clickable: true,
        dynamicBullets: true,
      },
      navigation: {
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev',
      },
    };
  }

  // 교육 활동: 3D 회전 (Y축 30도 회전 + 뒤로 500px 이동 + 그림자)
  function getEventsConfig() {
    return {
      effect: 'creative',
      grabCursor: true,
      loop: true,
      speed: 700,
      autoplay: {
        delay: 4000,
        disableOnInteraction: false,
        pauseOnMouseEnter: true,
      },
      creativeEffect: {
        perspective: true,
        limitProgress: 2,
        shadowPerProgress: true,  // 그림자 효과
        prev: {
          translate: [0, 0, -500],
          rotate: [0, 30, 0],  // Y축 30도 회전
          opacity: 0.5,
        },
        next: {
          translate: [0, 0, -500],
          rotate: [0, -30, 0],  // Y축 -30도 회전
          opacity: 0.5,
        },
      },
      pagination: {
        el: '.swiper-pagination',
        clickable: true,
        dynamicBullets: true,
      },
      navigation: {
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev',
      },
    };
  }

  console.log('📝 Swiper Creative 스크립트 v12 초기화 완료');
})();
